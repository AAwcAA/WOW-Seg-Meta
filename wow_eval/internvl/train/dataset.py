...
def preprocess_internvl2_5_with_mask(
        template_name,
        sources,
        tokenizer: transformers.PreTrainedTokenizer,
        num_image_token_list: list,
        ...
        ds_name: str = None,
        num_image: int = 1,
        mask_crop_id: torch.Tensor = None,
        num_masks: int = 1
) -> Dict:
    assert len(sources) == 1, 'process only the first conversations'
    conversations = sources[0]
        ...
        for conversation in conversations:
            if conversation['from'] == 'human':
                ...
                for i in range(image_cnt):
                    ...
                    # MASK_TOKEN = 49863
                    image_tokens = f'{IMG_START_TOKEN}{IMG_CONTEXT_TOKEN * num_image_token_list[current_image_idx]}{IMG_END_TOKEN}'
                    mask_tokens = f'<|mask|>{"MASK" * num_mask_token_list[current_image_idx]}<|mask|>' * num_masks
                    conversation['value'] = conversation['value'].replace('<image>', image_tokens + mask_tokens, 1)
                    current_image_idx += 1
            new_conversations.append(conversation)
        conversations = new_conversations
        assert current_image_idx == num_image, f'{current_image_idx} != {num_image}'
...
def dynamic_preprocess_with_mask(tensor, min_num=1, max_num=6, image_size=448, use_thumbnail=False):
    """
    Process a tensor with shape (1, w, h) using the same logic as dynamic_preprocess.
    Only keep tensors with non-zero sum and record their indices.
    
    Args:
        tensor: torch.Tensor with shape (1, w, h)
        min_num: minimum number of image blocks
        max_num: maximum number of image blocks  
        image_size: size of each image block
        use_thumbnail: whether to add a thumbnail image
    
    Returns:
        tuple: (list of torch.Tensor, list of int) - processed image tensors and their indices
    """
    # Ensure tensor has the correct shape
    if tensor.dim() != 3 or tensor.shape[0] != 1:
        raise ValueError(f"Expected tensor shape (1, w, h), got {tensor.shape}")
    
    # Get dimensions (remove the channel dimension for calculations)
    ...
    resized_tensor = F.interpolate(
        tensor.unsqueeze(0),  # Add batch dimension: (1, 1, h, w)
    ...
    for i in range(blocks):
        ...
        start_h = (i // (target_width // image_size)) * image_size
        start_w = (i % (target_width // image_size)) * image_size
        end_h = start_h + image_size
        end_w = start_w + image_size
        
        # Crop the tensor
        split_tensor = resized_tensor[:, start_h:end_h, start_w:end_w]
        
        # Check if tensor sum is not zero
        if split_tensor.sum() != 0:
            processed_tensors.append(split_tensor)
            valid_indices.append(i)
    
    if use_thumbnail and blocks != 1:
        # Create thumbnail tensor
        ...
        # Check if thumbnail tensor sum is not zero
        if thumbnail_tensor.sum() != 0:
            processed_tensors.append(thumbnail_tensor)
            valid_indices.append(i+1)
    
    return processed_tensors, valid_indices

def dynamic_preprocess_with_target_size(image, min_num=1, max_num=6, image_size=448, use_thumbnail=False):
    orig_width, orig_height = image.size
    aspect_ratio = orig_width / orig_height
    ...
    if use_thumbnail and len(processed_images) != 1:
        thumbnail_img = image.resize((image_size, image_size))
        processed_images.append(thumbnail_img)
    return processed_images, resized_img