...
import numpy as np
import torch
from internvl.model.internvl_chat.cascade_attention_mask import create_cascade_attention_mask
import pdb
IGNORE_INDEX = -100
...
def concat_pad_data_collator_with_cascade_attention_mask(features, max_item_length=None, pad_id=0):
    ...
    for idx in range(len(features)):
        ...
        feat['labels'] = temp_labels
        # feat['attention_mask'] = feat['input_ids'].ne(pad_id)
        feat['attention_mask'] = create_cascade_attention_mask(feat['input_ids'], cascade_config).unsqueeze(0)
    ...
    # Again, we will use the first element to figure out which key/values are not None for this model.
    for k, v in first.items():
        if k not in ('label', 'label_ids', 'pixel_values', 'image_flags', 'pixel_masks', 'thumbnail_values', 'crop_images', 'vaild_region_idx', 'mask_region_idx') and \
            ...
            if isinstance(v, torch.Tensor):
                ...
                batch[k] = torch.tensor([f[k] for f in features])
        if k in ('pixel_values', 'image_flags', 'pixel_masks', 'thumbnail_values', 'crop_images', 'mask_region_idx'):
            if isinstance(v, torch.Tensor):
                ...
                batch[k] = torch.concat([f[k] for f in features])
        if k in ('vaild_region_idx'):
            batch_list = []
            tmp = 0
            for f in features:
                batch_list.append(f[k] + tmp)
                tmp += (f[k] + 1)
            batch[k] = torch.tensor(batch_list, dtype=torch.long)
    return batch
