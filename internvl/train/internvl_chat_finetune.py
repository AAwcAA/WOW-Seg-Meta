...
from internvl.patch import (concat_pad_data_collator,
                            concat_pad_data_collator_with_cascade_attention_mask,
...
from internvl.train.dataset import (ConcatDataset, TCSLoader,
                                    ...
                                    dynamic_preprocess, dynamic_preprocess_with_target_size, preprocess,
                                    preprocess_internlm,
...
class DataTrainingArguments:
    ...
    conversation_template: str = field(
        default='',
        metadata={'help': 'The conversation template. Default is '},
    )
    scale_ratio: float = field(
        default=2.5,
        metadata={'help': 'Set the scale ratio for the image. Default is 2.5.'},
    )
    use_cascade_attention_mask: bool = field(
        default=True,
        metadata={'help': 'Set to True to use the cascade attention mask.'}
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def mask2token(self, pixel_masks, resize_img, sacle_ratio):
        pixel_masks = pixel_masks.unsqueeze(0)
        resize_pixel_masks = F.interpolate(pixel_masks.unsqueeze(0), size=(resize_img.size[1], resize_img.size[0]), mode='nearest').squeeze(0) # shape: [n_prompts, target_height, target_width]
        ones_coords = torch.nonzero(resize_pixel_masks.squeeze(0))
        ...
        # Scale the width and height by 2.5(default)
        new_width = min(max(width, height) * sacle_ratio, resize_img.size[0])
        new_height = min(max(width, height) * sacle_ratio, resize_img.size[1])

        # Calculate the new min and max coordinates
        new_min_col = max(0, int(center_col - new_width / 2))
        new_max_col = min(resize_img.size[0] - 1, int(center_col + new_width / 2))
        new_min_row = max(0, int(center_row - new_height / 2))
        new_max_row = min(resize_img.size[1] - 1, int(center_row + new_height / 2))

        # Adjust the center if the crop region exceeds the image boundaries
        if new_max_col - new_min_col + 1 < new_width:
            if new_min_col == 0:
                new_max_col = min(resize_img.size[0] - 1, int(new_min_col + new_width))
            else:
                new_min_col = max(0, int(new_max_col - new_width))

        if new_max_row - new_min_row + 1 < new_height:
            if new_min_row == 0:
                new_max_row = min(resize_img.size[1] - 1, int(new_min_row + new_height))
            else:
                new_min_row = max(0, int(new_max_row - new_height))

        # Crop the region from resize_img with the scaled bounding box
        cropped_img = resize_img.crop((new_min_col, new_min_row, new_max_col + 1, new_max_row + 1))
        # Crop the corresponding mask region
        cropped_mask = resize_pixel_masks[:, new_min_row:new_max_row + 1, new_min_col:new_max_col + 1]
        resize_cropped_mask = F.interpolate(cropped_mask.unsqueeze(0), size=(16, 16), mode='nearest').squeeze(0)
        mask_num_tokens = resize_cropped_mask.sum()
        return cropped_img, resize_cropped_mask, mask_num_tokens

    def multi_modal_multi_image_get_item_with_focus_mask_token_cascade_mask(self, data_item, scale_ratio):
        
        # Build transformation function
        transform = self.get_transform()

        rle_masks = data_item['masks']  
        n_prompts = len(rle_masks)
        pixel_masks = torch.from_numpy(mask_utils.decode(rle_masks)).permute(2, 0, 1) # shape: [n_prompts, h, w]
        ...
        if self.dynamic_image_size:  # If dynamic image size is enabled, preprocess the image dynamically
            image, resize_img = dynamic_preprocess_with_target_size(image, min_num=self.min_dynamic_patch,
                                        max_num=max(1, self.max_dynamic_patch),
                                        image_size=self.image_size, use_thumbnail=self.use_thumbnail)
            images += image
        ...
        pixel_values = [transform(image) for image in images]
        num_image_tokens = [self.num_image_token * num_tiles[0]]
        num_image = 1
        mask_region_idx = [0] * len(pixel_values)
        resize_cropped_mask_list = []

        for i in range(n_prompts):
            cropped_img, resize_cropped_mask, mask_num_tokens = self.mask2token(pixel_masks[i], resize_img, scale_ratio)
            num_image_tokens.append(mask_num_tokens)
            foreground_pixel_values = transform(cropped_img)
            pixel_values.append(foreground_pixel_values)
            num_image += 1
            mask_region_idx.append(1)
            resize_cropped_mask_list.append(resize_cropped_mask)

        resize_cropped_mask_list = torch.stack(resize_cropped_mask_list)
        pixel_values = torch.stack(pixel_values)
        ...
        data_item["conversations"][0]["value"] = self.CONVERSATION_TEMPLATE + "<image>" * (num_image - 1)
        ...
        #     'img_end': 151666,
        #     'prompt_delimiter': 25,  # ':'
        #     'output_separator': 1154,  # ' ,'
        #     'pad_token': 151643
        # }
        # attention_mask = create_custom_attention_mask_v10(temp_input_ids, cascade_config).unsqueeze(0)
        ...
        ret = dict(
            ...
            pixel_values=pixel_values,
            image_flags=torch.tensor([1] * num_patches, dtype=torch.long),
            pixel_masks=resize_cropped_mask_list,
            mask_region_idx=torch.tensor(mask_region_idx, dtype=torch.long)
        )
        return ret
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            ...
            try:
                ...
                if 'masks' in data_item:
                    ret = self.multi_modal_multi_image_get_item_with_focus_mask_token_cascade_mask(data_item, self.scale_ratio)
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        dataset = LazySupervisedDataset(
            ...
            scale_ratio=data_args.scale_ratio,
        )
...
def main():
    ...
    if data_args.use_packed_ds:
        if data_args.use_cascade_attention_mask:
            print(
                '============================================', "\n", 
                '============= use casacde mask =============', "\n",
                '============================================')
            collator = partial(
                packed_collate_fn,
                data_collator=concat_pad_data_collator_with_cascade_attention_mask,
        ...
        if data_args.use_cascade_attention_mask:
            print(
                ...
                '============= use casacde mask =============', "\n",
                '============================================')
            collator = concat_pad_data_collator_with_cascade_attention_mask
        else:
